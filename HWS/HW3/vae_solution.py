...
Original file is located at
    https://colab.research.google.com/drive/1aRnia9mWCmiU4gRsibwV3MFMZF0j6j7j
...
class DiagonalGaussianDistribution(object):
  ...
  def sample(self):
    ...
    smpl= torch.randn_like(self.mean)
    sample = self.mean + self.std * smpl      # WRITE CODE HERE
    return sample
  ...
  def kl(self):
    ...
    kl_div = 0.5 * torch.sum(torch.exp(self.logvar) + self.mean.pow(2) - self.logvar - 1, dim=1)  # WRITE CODE HERE
    return kl_div
  ...
  def nll(self, sample, dims=[1, 2, 3]):
    ...
    # c = -torch.log(self.std*(2*torch.pi)**0.5) -0.5*(sample-self.mean)
    # b = c**2 / self.var   # WRITE CODE HERE
    # negative_ll = torch.sum(-b, dim=dims).to(device)
    # return negative_ll
    negative_ll = -torch.log(self.std*(2*torch.pi)**0.5) -0.5*(sample-self.mean)**2/self.var
    return torch.sum(-negative_ll, dim=dims).to(device)
...
class VAE(nn.Module):
  ...
  def decode(self, z):
    ...
    conditional_dist = DiagonalGaussianDistribution(decoded, logvar=None)
  ...
  def sample(self, batch_size):
    ...
    m = torch.zeros((batch_size,self.z_dim))
    samples = DiagonalGaussianDistribution(m, logvar=None)
  ...
  def log_likelihood(self, x, K=100):
    ...
    for i in range(K):
      ...
      log_p_z = -prior.nll(z, dims=[1])
      log_p_x_given_z = -recon.nll(x)
      p_joint = log_p_z + log_p_x_given_z

      log_q_z_given_x = -posterior.nll(z, dims=[1])
      

      log_likelihood[:, i] = (p_joint - log_q_z_given_x) + torch.log(torch.tensor([1.0/K]).to(self.device))
      # WRITE CODE HERE (log of the summation terms in approximate log-likelihood, that is, log p_\theta(x, z_i) - log q_\phi(z_i | x))
      del z, recon
   
    
    ll = torch.logsumexp(log_likelihood, dim=[1]).to(self.device)    # WRITE CODE HERE (compute the final log-likelihood using the log-sum-exp trick)
    return ll
...
def interpolate(model, z_1, z_2, n_samples):
  ...
  z = lengths * (z_1 - z_2) +  z_2    # WRITE CODE HERE (interpolate z_1 to z_2 with n_samples points)
