...
class SimSiam(nn.Module):
    ...
    def loss (self, p1,p2,z1,z2, similarity_function='CosineSimilarity'):
        ...
        return (-(cos(p1, z2) + cos(p2, z1))).mean()/2
...
class CosineSimilarity(Module):
    ...
    def forward(self, x1: Tensor, x2: Tensor) -> Tensor:
        ...
        WRITE CODE HERE. DONT MODIFY THE PARAMETERS OF THE FUNCTION. Otherwise, tests might fail.
        """
        dot = torch.sum(x1 * x2, dim=self.dim)
        first_norm = torch.norm(x1, dim=self.dim)
        second_norm = torch.norm(x2, dim=self.dim)
        p = first_norm * second_norm
        if p[0] == 0:
            p = self.eps
        cos = dot / p
        return cos