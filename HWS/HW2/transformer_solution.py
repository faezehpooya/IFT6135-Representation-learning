...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Apply learned affine transformations
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        self.lin1 = nn.Linear(self.head_size* self.num_heads, 3 * self.head_size*self.num_heads)
        self.lin2 = nn.Linear(self.head_size* self.num_heads, self.head_size* self.num_heads)

    def get_attention_weights(self, queries, keys, mask=None):
        # Project the queries, keys, and values
        attn_logits = torch.matmul(queries, keys.transpose(2,3)) / math.sqrt(self.head_size)
        if mask is not None:
            if mask.shape[-1] != attn_logits.shape[-1]:
    ...
    def forward(self, hidden_states, mask=None):
        ...
        qkv = self.lin1(hidden_states)
        qkv = self.split_heads(qkv)
        q, k, v = qkv.chunk(3, dim = -1)
        attention = self.apply_attention(q, k, v, mask)
        out = self.lin2(attention)
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # TODO: Write your code here
        # ==========================
        x_normalized = self.layer_norm_1(x)
        attention_outputs = self.attn(x_normalized, mask)
        x_new = x+attention_outputs
        attention_outputs_normalized = self.layer_norm_2(x_new)
        outputs = self.linear(attention_outputs_normalized)
        out =  outputs + x_new
        return out
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # ==========================
        cls_token_rep = x[:, 0, :]
        output = self.mlp_head(cls_token_rep)
