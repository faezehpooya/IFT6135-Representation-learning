...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        # print("im in patch_emp and self.proj is")
        # print(x.shape)
        x = x.flatten(2).transpose(1, 2)  # BCHW -> BNC
        # print("im in patchemp and flatten is")
        # print(x.shape)
        return x
...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        x = self.fc1(x)
        # print("im in mlp after fc1 and shape x is")
        # print(x.shape)
        x = self.act(x)
        # print("im in mlp after act and shape x is")
        # print(x.shape)
        x = self.drop1(x)
        # print("im in mlp after drop1 and shape x is")
        # print(x.shape)
        x = self.fc2(x)
        # print("im in mlp after fc2 and shape x is")
        # print(x.shape)
        x = self.drop2(x)
        # print("im in mlp after drop2 and shape x is")
        # print(x.shape)
        return x
...
class MixerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.drop_path = nn.Identity() #droppath = 0

    def forward(self, x):
        # print("im in mixer block and norm1 is:")
        # print(self.norm1(x).shape)
        x = x + self.drop_path(self.mlp_tokens(self.norm1(x).transpose(1, 2)).transpose(1, 2))
        # print("im in MixerBlock after first line and shape x is")
        # print(x.shape)
        # print("im in mixer block and norm2 is:")
        # print(self.norm2(x).shape)
        x = x + self.drop_path(self.mlp_channels(self.norm2(x)))
        # print("im in MixerBlock after second line and shape x is")
        # print(x.shape)
        return x
...
class MLPMixer(nn.Module):
    ...
    def forward(self, images):
        ...
        # print("im in MLPMixer after patchemp line and shape x is")
        # print(x.shape)
        x = self.blocks(x)
        # print("im in MLPMixer after blocks line and shape x is")
        # print(x.shape)
        x = self.norm(x)
        # print("im in MLPMixer after norm line and shape x is")
        # print(x.shape)
        x = x.mean(dim=1)
        # print("im in MLPMixer after mean and the shape is:")
        # print(x.shape)
        x = self.head(x)
        # print("im in MLPMixer after head line and shape x is")
        # print(x.shape)
        return x
    ...
    def visualize(self, logdir):
        ...
        patches = weights.view(-1,8,8)
        for token in range(128):
          plt.subplot(8,16,token+1)
          plt.xticks([])
          plt.yticks([])
          plt.imshow(patches[token].detach().cpu().numpy())
        plt.show()
        plt.savefig(f"{logdir}/output.png")
